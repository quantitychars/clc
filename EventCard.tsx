...
export default function EventCard({ event, matchScore }: { event: CampusEvent; matchScore?: number }) {
  return (
    <article className={`rounded-lg border border-border bg-card p-4 shadow-sm border-l-4 ${categoryColors[event.category] || ""} animate-fade-in`} aria-label={event.title}>
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <h3 className="font-semibold text-foreground">{event.title}</h3>
          <p className="text-sm text-muted-foreground mt-1">
            <time dateTime={event.date}>{new Date(event.date).toLocaleDateString("en-IE")}</time> · {event.room}
          </p>
        ...
        {matchScore !== undefined && (
          <span className="shrink-0 rounded-pill bg-primary px-3 py-1 text-xs font-bold text-primary-foreground" aria-label={`${matchScore}% interest match`}>
            {matchScore}% match
          </span>
        )}
      </div>
